...
Mapper = $.klazz({
  ...
  initialize: function(xsize, ysize, room_templates) {
    this.xsize = parseInt(xsize, 10)-3;
    this.ysize = parseInt(ysize, 10)-5;
      ...
      $.each(rooms, function(index, room) {
        ...
        $.each(room.coded_coords, function(y_in_room, room_y_row) {
          $.each(room_y_row, function(x_in_room, room_value) {
            digCallback(x + x_in_room, y + y_in_room + extra_y_offset, parseInt(room_value)); /////////////
  ...
  surround_every_floor_with_wall: function() {
    ...
    $.each(this.coded_coords, function(y, y_row) {
      $.each(y_row, function(x, tile) {
        if(tile === FLOOR) {
          if(the_mapper.north_from(x,y) === ' ')     the_mapper.wall_around_corridor(x,y-1);
          if(the_mapper.south_from(x,y) === ' ')     the_mapper.wall_around_corridor(x,y+1);
          if(the_mapper.west_from(x,y) === ' ')      the_mapper.wall_around_corridor(x-1,y);
          if(the_mapper.east_from(x,y) === ' ')      the_mapper.wall_around_corridor(x+1,y);
          if(the_mapper.northeast_from(x,y) === ' ') the_mapper.wall_around_corridor(x+1,y-1);
          if(the_mapper.southeast_from(x,y) === ' ') the_mapper.wall_around_corridor(x+1,y+1);
          if(the_mapper.northwest_from(x,y) === ' ') the_mapper.wall_around_corridor(x-1,y-1);
          if(the_mapper.southwest_from(x,y) === ' ') the_mapper.wall_around_corridor(x-1,y+1);
        } else if(tile === DOOR) {
...
ROT.Map.Rangersheck = function (width, height) {
   return new Mapper(width, height, [
        ...
        new RoomTemplate(  '  111  \n'
                         ...
                         + ' 10001 \n'
                         + '  111  \n'),
        new RoomTemplate(  ' 111111111111111 \n'
                         + '10000000000000001\n'
                         + '10000000000000001\n'
                         + '10000111211100002\n'
                         + '10000100000100001\n'
